...
impl Parser {
    ...
    fn parse_primary(&mut self) -> Result<Expr, SlimeError> {
        match &self.current {
            Token::Integer(n) => { let v = *n; self.advance(); Ok(Expr::Integer(v)) }
    ...
    fn parse_stmt(&mut self) -> Result<Stmt, SlimeError> {
        match &self.current {
            Token::Let => {
                ...
                Ok(Stmt::Let(name, Some(ty), expr))
            }
            Token::Return => {
                self.advance();
                let expr = if self.current == Token::Semicolon { None } else { Some(Box::new(self.parse_expr()?)) };
                self.expect(Token::Semicolon)?;
                Ok(Stmt::Return(expr))
            }
            _ => {
